...
void check_elf(unsigned char *e_ident)
{
	...
	{
		if (e_ident[counter] != 127 && e_ident[index] != 'E' &&
		    e_ident[index] != 'L' &&
		    e_ident[index] != 'F')
		{
...
/**
 * print_magic - Now this prints the magic numbers of an ELF header.
...
/**
 * print_data - A fn that prints the data of an ELF header.
...
/**
 * print_version - this fn Prints the version of an ELF header.
...
/**
 * print_abi - thispPrints the ABI version of an ELF header.
 ...
 *
 * Description: for a file tha can't be closed - exit code 98.
 */
void close_elf(int elf)
{
	if (close(elf) == -1)
